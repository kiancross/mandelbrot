...
public class MandelbrotSetIterationCountGenerator {
  ...
  /**
   * Bound representing the minimum and maximum complex number to
   * have iteration values calculated for.
  */
  final Bound bound;

  /**
   * Maximum number of iterations before assuming the complex number
  ...
  */
   ...
   *
   * @param bound Bound representing the minimum and maximum complex
   * number to have iteration values calculated for.
   *
   * @param maximumIterations Maximum number of iterations before assuming the
   * complex number lies within the mandelbrot set.
   *
   * @param escapeRadius The escape radius to use when calculating the iteration 
   * values.
  */
  public MandelbrotSetIterationCountGenerator(
    final Bound bound,
    final int maximumIterations,
    final double escapeRadius
  ) {
  ...
  /**
   * Calculate the number of iterations required for the given complex
   * number to exceed the escape radius.
   *
   * @param c The complex number to be used in the mandelbrot iterative
   * calculation.
   *
   * @param The number of iterations taken to exceed the radius, or -1 if
  ...
  */
  ...
  /**
   * Gets a subtask of the calculation which can be used to run the
  ...
  */
  private Runnable getSubtask(
    ...
    return new Runnable() {
      public void run() {
        ...
        final double realStepSize = (maximumC.getReal() - minimumC.getReal()) / (double) xResolution;

        final double imaginaryStepSize = (maximumC.getImaginary() - minimumC.getImaginary()) /
          (double) yResolution;
        ...
        final int xEnd = (int) Math.ceil((threadNumber + 1) * (xResolution / (double) totalThreads));
  ...
  /**
   * Calculate the iteration values for a given resolution using a certain
   * number of threads.
   *
   * @param xResolution The resolution of the x-axis.
   * @param yResolution The resolution of the y-axis.
   ...
   *
   * @return A 2D array containing the iteration values. Each position in the
   * array can be accessed as so: array[x][y].
  */
  public int[][] calculate(
    ...
    for (int i = 0; i < numberOfThreads; i++) {
      ...
      final Thread thread = new Thread(getSubtask(
        set, numberOfThreads, i, xResolution, yResolution
      ));
